#include <stdio.h>
#include <iostream>
#include <stdlib.h>
#include <string.h>
using namespace std;

// The number of columns comprising a state in AES. This is a parameter
// that could be 4, 6, or 8.  For this example we set it to 4.
#define col_state 4

// The number of rounds in AES Cipher. It is initialized to zero. 
// The actual value is computed from the input.
int rounds=0;

// The number of 32 bit words in the key. It is initialized to zero. 
// The actual value is computed from the input.
int length_key_bit=0;

// in - the array that holds the plain text to be encrypted.
// out - the array that holds the cipher text.
// state - the array that holds the intermediate results during encryption.
unsigned char input_arr[1024], output_arr[1024], state[4][col_state];

// The array that stores the round keys.
unsigned char RoundKey[240];

// The Key input to the AES Program
unsigned char Key[32];

int getSBoxValue(int num) {
   int sbox[256] = {
       };
   return sbox[num];
}

// The round constant word array, roundconst[i], contains the values given by 
// x to the power (i-1) being powers of x (x is denoted as {02}) in the 
// field GF(28).  Note that i starts at 1, not 0).
int roundconst[255] = {
           };

// This function produces col_state(rounds+1) round keys. The round keys are used in 
// each round to encrypt the states. 
void KeyExpansion() {
   int i,j;
   unsigned char temp[4],k;
	
   // The first round key is the key itself.
   for (i=0 ; i < length_key_bit ; i++) {
      RoundKey[i*4] = Key[i*4];
      RoundKey[i*4+1] = Key[i*4+1];
      RoundKey[i*4+2] = Key[i*4+2];
      RoundKey[i*4+3] = Key[i*4+3];
   }

   // All other round keys are found from the previous round keys.
   while (i < (col_state * (rounds+1))) {
      for (j=0 ; j < 4 ; j++) {
	 temp[j] = RoundKey[(i-1) * 4 + j];
      }
      if (i % length_key_bit == 0) {
	 // This function rotates the 4 bytes in a word to the left once.
	 // [a0,a1,a2,a3] becomes [a1,a2,a3,a0]
	 
	 // Function RotWord()
	 k = temp[0];
	 temp[0] = temp[1];
	 temp[1] = temp[2];
	 temp[2] = temp[3];
	 temp[3] = k;
	 
	 // SubWord() is a function that takes a four-byte input word and 
	 // applies the S-box to each of the four bytes to produce an output
         // word.
	 
	 // Function Subword()
	 temp[0] = getSBoxValue(temp[0]);
	 temp[1] = getSBoxValue(temp[1]);
	 temp[2] = getSBoxValue(temp[2]);
	 temp[3] = getSBoxValue(temp[3]);

	 temp[0] =  temp[0] ^ roundconst[i/length_key_bit];
      } else if (length_key_bit > 6 && i % length_key_bit == 4) {
	 // Function Subword()
	 temp[0] = getSBoxValue(temp[0]);
	 temp[1] = getSBoxValue(temp[1]);
	 temp[2] = getSBoxValue(temp[2]);
	 temp[3] = getSBoxValue(temp[3]);
      }
      RoundKey[i*4+0] = RoundKey[(i-length_key_bit)*4+0] ^ temp[0];
      RoundKey[i*4+1] = RoundKey[(i-length_key_bit)*4+1] ^ temp[1];
      RoundKey[i*4+2] = RoundKey[(i-length_key_bit)*4+2] ^ temp[2];
      RoundKey[i*4+3] = RoundKey[(i-length_key_bit)*4+3] ^ temp[3];
      i++;
   }
}

// This function adds the round key to state.
// The round key is added to the state by an XOR function.
void AddRoundKey(int round) {
   int i,j;
   for (i=0 ; i < col_state ; i++) {
      for (j=0 ; j < 4 ; j++) {
	 state[j][i] ^= RoundKey[round * col_state * 4 + i * col_state + j];
      }
   }
}

// The SubBytes Function Substitutes the values in the
// state matrix with values in an S-box.
void SubBytes() {
   int i,j;
   for (i=0 ; i < 4 ; i++) {
      for (j=0 ; j < col_state ; j++) {
	 state[i][j] = getSBoxValue(state[i][j]);
      }
   }
}

// The ShiftRows() function shifts the rows in the state to the left.
// Each row is shifted with different offset.
// Offset = Row number. So the first row is not shifted.
void ShiftRows() {
   unsigned char temp;

   // Rotate first row 1 columns to left	
   temp = state[1][0];
   state[1][0] = state[1][1];
   state[1][1] = state[1][2];
   state[1][2] = state[1][3];
   state[1][3] = temp;

   // Rotate second row 2 columns to left	
   temp = state[2][0];
   state[2][0] = state[2][2];
   state[2][2] = temp;

   temp = state[2][1];
   state[2][1] = state[2][3];
   state[2][3] = temp;

   // Rotate third row 3 columns to left
   temp = state[3][0];
   state[3][0] = state[3][3];
   state[3][3] = state[3][2];
   state[3][2] = state[3][1];
   state[3][1] = temp;
}

// xtime is a macro that finds the product of {02} and the argument to
// xtime modulo {1b}  
#define xtime(x)   ((x<<1) ^ (((x>>7) & 1) * 0x1b))

// MixColumns function mixes the columns of the state matrix
void MixColumns() {
   int i;
   unsigned char Tmp,Tm,t;
   for (i=0 ; i < col_state ; i++) {	
      t = state[0][i];
      Tmp = state[0][i] ^ state[1][i] ^ state[2][i] ^ state[3][i] ;
      Tm = state[0][i] ^ state[1][i] ; 
      Tm = xtime(Tm); 
      state[0][i] ^= Tm ^ Tmp ;
      
      Tm = state[1][i] ^ state[2][i] ; 
      Tm = xtime(Tm); 
      state[1][i] ^= Tm ^ Tmp ;

      Tm = state[2][i] ^ state[3][i] ; 
      Tm = xtime(Tm); 
      state[2][i] ^= Tm ^ Tmp ;

      Tm = state[3][i] ^ t ; 
      Tm = xtime(Tm); 
      state[3][i] ^= Tm ^ Tmp ;
   }
}

// Cipher is the main function that encrypts the PlainText.
void Cipher() {
   int i,j,round=0;

   //Copy the input PlainText to state array.
   for (i=0 ; i < col_state ; i++) {
      for (j=0 ; j < 4 ; j++) {
	 state[j][i] = input_arr[i*4 + j];
      }
   }

   // Add the First round key to the state before starting the rounds.
   AddRoundKey(0); 
	
   // There will be rounds rounds.
   // The first rounds-1 rounds are identical.
   // These rounds-1 rounds are executed in the loop below.
   for (round=1 ; round < rounds ; round++) {
      SubBytes();
      ShiftRows();
      MixColumns();
      AddRoundKey(round);
   }
	
   // The last round is given below.
   // The MixColumns function is not here in the last round.
   SubBytes();
   ShiftRows();
   AddRoundKey(rounds);
   
   // The encryption process is over.
   // Copy the state array to output array.
   for (i=0 ; i < col_state ; i++) {
      for (j=0 ; j < 4 ; j++) {
	 output_arr[i*4+j]=state[j][i];
      }
   }
}

int fillBlock (int sz, char *str, unsigned char *in) {
   int j=0;
   while (sz < strlen(str)) {
      if (j >= col_state*4) break;
      in[j++] = (unsigned char)str[sz];
      sz++;
   }
   // Pad the block with 0s, if necessary
   if (sz >= strlen(str)) for ( ; j < col_state*4 ; j++) in[j] = 0;
   return sz;   
}

int main(int argc, char **argv) {
   int i;
   
   if (argc != 2) {
      cerr << "How to use the AES encryption\n: " << argv[0] << " '< enter keysize(eg. './csb20021_aes 1' for 128 bit key) as command line argument 1=128 or 2=192 or 3=256>'\n";
      exit(0);
   }
   switch (atoi(argv[1])) {
   case 1: length_key_bit = 4; break;
   case 2: length_key_bit = 6; break;
   case 3: length_key_bit = 8; break;
   default: length_key_bit = 4; break;
   }

   // Calculate rounds from length_key_bit and, implicitly, from col_state
   rounds = length_key_bit + 6;

   // The key values are placed here
   Key[0]  = 0x2b;  Key[1]  = 0x7e;  Key[2]  = 0x15;  Key[3]  = 0x16;
   Key[4]  = 0x28;  Key[5]  = 0xae;  Key[6]  = 0xd2;  Key[7]  = 0xa6;
   Key[8]  = 0xab;  Key[9]  = 0xf7;  Key[10] = 0x15;  Key[11] = 0x88;
   Key[12] = 0x09;  Key[13] = 0xcf;  Key[14] = 0x4f;  Key[15] = 0x3c;

   // Get the input string
   cout<<"Enter your string here[max 1000 characters]:\n";
   char str[1024];
   fgets(str, 1024, stdin);
   cout<<"Key size used is: "<< argv[1] << "\n";

   // The KeyExpansion routine is called before encryption.
   KeyExpansion();
      
   // sz is the cursor into the input string
   int sz=0;
   // Each iteration encrypts one block = col_state*4 bytes = 128 bits in this case
   while (sz < strlen(str)) {
      // Fill the array 'in' with the next plaintext block
      sz = fillBlock (sz, str, input_arr);

      // The block is encrypted here - the result is in the array 'out'
      Cipher();

      // Output the encrypted block.
      cout<<"The AES encrypted block is:\n";
      for (i=0 ; i < col_state*4 ; i++) cout << (int)output_arr[i] << " ";
   }
   printf("\n\n");
}