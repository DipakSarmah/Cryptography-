...
int SBoxfunc(int num,int row,int column){\
...
void Cipher(long long int msg,long long int key){
	...
	final=final<<32;
	final+=r0;
	cout<<"Ciphertext: "<<hex<<final<<"\nChipher INT:=  "<<l0<<" --  Right INT:= "<<r0 <<endl;
...
int main (int argc, char *argv[]){
	...
	//long long int keya[8]={56,98,121,116,101,107,101,121};//randomized key generated and input into the Array
	cout<<"key is generated randomly\n";
    long long int keya[8];
    for(int i=0;i<8;i++){
        keya[i]=RandInt()%200;
    }
	//change the key anytime and anyhow we want
	long long int msga[8]={109,101,115,115,97,103,101,46}; //the msg input can be changed accordingly as the Key
